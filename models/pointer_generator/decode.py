...
from models.pointer_generator import beam_search
...
class BeamSearchDecoder(object):
  ...
  def decode(self):
    ...
    while True:
      ...
      try:
        fst_stop_idx = decoded_words.index(config.END_TOKEN) # index of the (first) [STOP] symbol
  ...
  def write_for_bleu(self, tars, preds):
    tf.logging.info('Saving decode results to files:.')
    tar_file = os.path.join(self._decode_dir, "%s.tgt" % FLAGS.exp_name)
    pred_file = os.path.join(self._decode_dir, "%s.pred" % FLAGS.exp_name)
    tf.logging.info(tar_file)
    tf.logging.info(pred_file)

    tar_outobj = open(tar_file, 'w')
    pred_outobj = open(pred_file, 'w')
    for index, tar in enumerate(tars):
      pred = preds[index]
      tar_outobj.write(tar + '\n')
      write_line = ' '.join(pred)
      pred_outobj.write(write_line + '\n')
