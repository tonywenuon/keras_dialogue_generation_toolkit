...
def run_beam_search(sess, model, vocab, batch):
  ...
  hyps = [Hypothesis(tokens=[vocab.word2id(config.START_TOKEN)],
                     log_probs=[0.0],
                     state=dec_in_state,
                     attn_dists=[],
                     p_gens=[],
  ...
  while steps < FLAGS.max_dec_steps and len(results) < FLAGS.beam_size:
    ...
    latest_tokens = [t if t in range(vocab.size()) else vocab.word2id(config.UNK_TOKEN) for t in latest_tokens] # change any in-article temporary OOV ids to [UNK] id, so that we can lookup word embeddings
    ...
    for h in sort_hyps(all_hyps): # in order of most likely h
      if h.latest_token == vocab.word2id(config.END_TOKEN): # if stop token is reached...
