...
class SrcFactTransformerCoordinateEmbedding(Layer):
    ...
    def get_config(self):
        ...
        config['max_conv_depth'] = self.max_conv_depth 
        # 
        return config
    ...
    def build(self, input_shape):
        ...
        self.sf_depth_embeddings = self.add_weight(
            ...
            trainable=True)
        if self.max_conv_depth != 0:
            self.conv_depth_embeddings = self.add_weight(
                shape=(self.max_conv_depth, d_model),
                initializer='uniform',
                name='conv_depth_position_embeddings',
                trainable=True)
    ...
    def call(self, inputs, **kwargs):
        ...
        if sf_depth is not None:
            if sf_depth == 0:
                ...
                result = K.permute_dimensions(result, [0, 2, 1, 3])
                result = result + self.conv_depth_embeddings[:, :]
                result = K.permute_dimensions(result, [0, 2, 1, 3])
